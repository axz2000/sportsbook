...
def gameToday():
	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/53352.3.json').json()
...
def fetch():
  try:
  	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/53352.3.json').json() #gives the game id
  except:
...
def picks(): #this needs some work/checking
	...
	resulting['League'] = ['FDA']*len(resulting['Bet State Chosen'])
