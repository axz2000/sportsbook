...
def gameToday():
	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/49693.3.json').json()
...
def fetch():
  try:
  	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/49693.3.json').json() #gives the game id
  except:
...
def fetchName(): 
  ...
  '''epl = epl[epl.ID != 'Rotherham']
  epl = epl[epl.ID != 'Derby County']
  epl = epl[epl.ID != 'Draw Rotherham v Derby County']'''
  indexer = []
  for i in range(int(len(epl)/3)):
  	indexer += [i]*3
  epl['gameNum'] = indexer
  return epl
...
def picks(): #this needs some work/checking
	...
	resulting['League'] = ['ECL']*len(resulting['Bet State Chosen'])
