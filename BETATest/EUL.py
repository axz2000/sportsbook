...
def gameToday():
	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/49689.3.json').json()
...
def fetch():
  try:
  	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/49689.3.json').json() #gives the game id
  except:
...
def fetchName(): 
  url = 'https://projects.fivethirtyeight.com/soccer-predictions/europa-league/'
  #print('hello')
...
def picks(): #this needs some work/checking
	...
	resulting['League'] = ['EUL']*len(resulting['Bet State Chosen'])
