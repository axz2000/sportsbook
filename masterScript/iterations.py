...
def powerLaw(portfolioAmt,df):
  ...
  allocation1 = [np.minimum((portfolioAmt*np.median(kelly)**2)*(i/np.sum(kelly)), 0.3*portfolioAmt) for i in kelly] #RISK TOLERANCE ESTABLISHED HERE 
...
def dailyReturn():
	if (input("Are you here to update? ").lower() == 'yes'):
		...
		bettingHistory = bet.reset_index()
		bettingHistory.to_csv(os.getcwd() + '/historicalBetLedger.csv', mode='a', header=False, index = False)
