...
def gameToday():
	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/53228.3.json').json()
...
def fetch():
  try:
  	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/53228.3.json').json() #gives the game id
  except:
...
def fetchName(): 
  url = 'https://projects.fivethirtyeight.com/soccer-predictions/superligaen/'
  #print('hello')
...
def picks(): #this needs some work/checking
	...
	resulting['League'] = ['DSL']*len(resulting['Bet State Chosen'])
